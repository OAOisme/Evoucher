...
const See = ({ navigation, route }) => {
    ...
    const getData = async (thedate) => {
        try {
            let das = await axios.post(`API_URI/vouchershow/${route.params.info._id}`, {
    ...
    const fetchData = async () => {
        try {
            axios.get(`API_URI/vouchershow/${route.params.info._id}`).then(das => {
                if (das.status == 200) {
                    setData(das.data)
                }
            })
